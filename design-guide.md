...
### Toast Notifications
Used for temporary, auto-dismissing success/error messages (e.g., HTMX out-of-band updates). They appear at the bottom right (`fixed bottom-4 right-4 z-[60]`).

**1. Success Toast Example**
```html
<div class="flex items-center p-4 w-full max-w-xs text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 border-l-4 border-green-500 transition-opacity duration-300">
    <div class="ml-3 text-sm font-normal">Settings updated successfully.</div>
...
```
- **Borders:** `border-l-4 border-green-500` (Success), `border-red-500` (Error), `border-blue-500` (Info)
